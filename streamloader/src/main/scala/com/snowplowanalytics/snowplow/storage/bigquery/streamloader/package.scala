...
package object streamloader {
  ...
  implicit val badRowEncoder: MessageEncoder[BadRow] = {
    br => Right(br.compact.getBytes())
  }

  implicit val shreddedTypesEncoder: MessageEncoder[Set[ShreddedType]] = {
    t => Right(toPayload(t).noSpaces.getBytes())
  }

  implicit val messageEncoder: MessageEncoder[FailedInsert] = {
    tr => Right(tr.tableRow.getBytes())
  }
}
