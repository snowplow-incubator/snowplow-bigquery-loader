...
import com.snowplowanalytics.snowplow.storage.bigquery.fs2loader.IdInstances._
import com.google.api.services.bigquery.model.TableRow

/** Row ready to be passed into Loader stream and Mutator topic */
case class Fs2LoaderRow(collectorTstamp: Instant, data: TableRow, inventory: Set[ShreddedType])
...
object Fs2LoaderRow {
  ...
  /**
    ...
    */
  def parse(resolver: Json)(record: String): Either[BadRow, Fs2LoaderRow] =
    for {
    ...
    } yield Fs2LoaderRow(tstamp, row, event.inventory)
  ...
  def fromEvent(resolver: Resolver[Id])(event: Event): Either[BadRow, (TableRow, Instant)] = {
    ...
    } yield ((atomic ++ jsons).foldLeft(new TableRow())((r, kv: (String, Any)) => r.set(kv._1, kv._2)), timestamp) // Mimic the com.spotify.scio.bigquery.TableRow constructor
  }
