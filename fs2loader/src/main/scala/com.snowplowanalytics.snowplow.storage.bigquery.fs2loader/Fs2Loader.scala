...
import com.snowplowanalytics.snowplow.storage.bigquery.fs2loader.Source.PubsubSource
...
//                                                                                         +--------+
  ...
  def run(e: Environment)(implicit cs: ContextShift[IO], c: Concurrent[IO], T: Timer[IO]): IO[ExitCode] = {
    ...
    val eventStream: Stream[IO, Either[BadRow, Fs2LoaderRow]] =
      source.getStream.map(Fs2LoaderRow.parse(e.resolverJson))

    for {
      client <- Bigquery.getClient
      queue  <- Queue.bounded[IO, Set[ShreddedType]](MaxConcurrency)
      sinkBadGood = eventStream.observeEither[BadRow, Fs2LoaderRow](
        badSink(e),
  ...
  /**
    ...
    */
  def bigquerySink(e: Environment, c: BigQuery): Pipe[IO, Fs2LoaderRow, Unit] =
  ...
  /**
    ...
    * @param getTypes     A function to extract types from a [[Fs2LoaderRow]].
  ...
  def goodSink(
    ...
    getTypes: Fs2LoaderRow => Set[ShreddedType],
    bigquerySink: Pipe[IO, Fs2LoaderRow, Unit]
  ): Pipe[IO, Fs2LoaderRow, Unit] =
