...
object Loader {
  ...
  def run(env: LoaderEnvironment, sc: ScioContext): Unit = {
    ...
    lazy val stackdriverConfig = new StackdriverConfig {
      ...
      override def resourceLabels(): util.Map[String, String] = Map[String, String]("project_id" -> projectId(), "job_name" -> s"bq-loader-${env.common.config.name}", "region" -> "europe-west1").asJava
    }
    @transient
