...
class MutatorCliSpec extends Specification {
  "parse" should {
    "extract valid configuration for listen subcommand" in {
      val expected = ListenCommand(SpecHelpers.mutatorEnv, false)
      val result =
    ...
    "extract valid configuration for create subcommand" in {
      val expected = CreateCommand(SpecHelpers.mutatorEnv)
      val result =
    ...
    "extract valid configuration for add-column subcommand" in {
      val expected = AddColumnCommand(SpecHelpers.mutatorEnv, SpecHelpers.schema, SpecHelpers.property)
