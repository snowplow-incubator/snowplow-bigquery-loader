...
import com.snowplowanalytics.snowplow.storage.bigquery.common.config.model.Config.RepeaterOutputs
import com.snowplowanalytics.snowplow.storage.bigquery.common.config.model.{Config, Input, Output}
...
object SpecHelpers {
  ...
  val subscription: String             = "failed-inserts-sub"
  ...
  val deadLetters: Output.Gcs          = Output.Gcs("gs://some-bucket/")
  val output: RepeaterOutputs          = RepeaterOutputs(good, deadLetters)
  val repeater: Config.Repeater        = Config.Repeater(input, output)
  val projectId: String                = "snowplow-data"
  val repeaterEnv: RepeaterEnvironment = RepeaterEnvironment(repeater, jsonResolver, projectId)
}
