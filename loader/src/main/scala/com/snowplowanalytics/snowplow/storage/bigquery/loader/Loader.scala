...
object Loader {
  ...
  def run(env: LoaderEnvironment, sc: ScioContext): Unit = {
    ...
    lazy val stackdriverConfig = new StackdriverConfig {
      ...
      override def resourceLabels(): util.Map[String, String] = Map[String, String]("job_name" -> env.common.config.name, "region" -> sc.options.outputRuntimeOptions().get("region").toString).asJava
    }
    @transient
