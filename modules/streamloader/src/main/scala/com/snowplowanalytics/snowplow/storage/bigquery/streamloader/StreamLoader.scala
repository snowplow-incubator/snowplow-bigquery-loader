...
object StreamLoader {
  ...
  def run[F[_]: ContextShift : ConcurrentEffect: Timer : Logger](e: LoaderEnvironment): F[ExitCode] =
    Resources.acquire(e).use { resources =>
      ...
      Logger[F].info(s"BQ Stream Loader ${generated.BuildInfo.version} has started. Listening ${e.config.input.subscription}") *>
        runWithShutdown(e.config.terminationTimeout, resources.sentry, eventStream.concurrently(metrics), sink).attempt.flatMap {
          case Right(_) =>
   ...
   * and then we would not be able to ack the outstanding records.
   */
  private def runWithShutdown[F[_]: Concurrent: Timer: Logger, A](timeout: FiniteDuration, sentry: Sentry[F], source: Stream[F, A], sink: Pipe[F, A, Unit]): F[Unit] =
  ...
  private def terminateStream[F[_]: Concurrent: Timer: Logger, A](queue: NoneTerminatedQueue[F, A], fiber: Fiber[F, Unit], timeout: FiniteDuration): F[Unit] =
    for {
