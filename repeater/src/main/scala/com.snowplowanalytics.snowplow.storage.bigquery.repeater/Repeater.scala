    ...
    RepeaterCli.parse(args) match {
      case Right(command) =>
        val process = for {
          ...
          events     = services.PubSub.getEvents(resources.env.config.projectId, resources.serviceAccount, command.failedInsertsSub, resources.client).interruptWhen(resources.stop)
          processing = Flow.process(resources)(events)
          logging    = Stream.awakeEvery[IO](1.minute).evalMap(_ => resources.showStats)
